...
public class RedisUtils {
    ...
    public boolean expire(String key, long time) {
        if (time <= 0) {
            throw new BusinessException(ResultCodeEnums.VAILD_PARAMETER, "请设置合理过期时间");
        }
        try {
    ...
    public long incr(String key, long delta) {
        if (delta < 0) {
            throw new BusinessException(ResultCodeEnums.VAILD_PARAMETER, "递增因子必须大于0");
        }
    ...
    public long decr(String key, long delta) {
        if (delta < 0) {
            throw new BusinessException(ResultCodeEnums.VAILD_PARAMETER, "递减因子必须大于0");
        }
        return redisTemplate.opsForValue().increment(key, -delta);
    }
